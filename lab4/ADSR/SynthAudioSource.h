...
struct SineWaveVoice : public juce::SynthesiserVoice
{
    SineWaveVoice(const juce::AudioSampleBuffer& wavetableToUse, unsigned int voices,
        float* ptrA, float* ptrD, float* ptrS, float* ptrR) : wavetable(wavetableToUse)
    {
...
public:
    SynthAudioSource(juce::MidiKeyboardState& keyState,
        float* ptrA, float* ptrD, float* ptrS, float* ptrR) : keyboardState(keyState)
    {
        for (auto i = 0; i < numVoices; ++i)
            synth.addVoice(new SineWaveVoice(signalTable, numVoices, ptrA, ptrD, ptrS, ptrR));
