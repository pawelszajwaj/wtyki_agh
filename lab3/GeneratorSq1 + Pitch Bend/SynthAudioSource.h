...
struct SineWaveVoice : public juce::SynthesiserVoice
{
    SineWaveVoice(const juce::AudioSampleBuffer& wavetableToUse, unsigned int voices,
        float *ptrpitchBend) : wavetable(wavetableToUse)
    {
...
{
public:
    SynthAudioSource(juce::MidiKeyboardState& keyState, float* ptrpitchBend)
        : keyboardState(keyState)
    {
        for (auto i = 0; i < numVoices; ++i)
            synth.addVoice(new SineWaveVoice(signalTable, numVoices, ptrpitchBend));
